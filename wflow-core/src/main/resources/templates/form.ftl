<#if isAuthorize>
    ...
    <#if isRecordExist >
        <#if !element.parent??>
        <form id="${element.properties.id!}" name="${element.properties.id!}" class="form-horizontal form-container <#if element.properties.readonly! == 'true'>readonly</#if>" ${elementMetaData!} <#if element.properties.url??>action="${element.properties.url}"</#if> method="POST" enctype="multipart/form-data">
        </#if>
