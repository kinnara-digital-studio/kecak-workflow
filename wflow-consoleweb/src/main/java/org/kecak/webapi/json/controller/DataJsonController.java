...
public class DataJsonController implements Declutter {
	...
	@RequestMapping(value = "/json/data/app/(*:appId)/(~:appVersion)/datatablelist/(*:dataListId)", method = RequestMethod.POST)
	public void getListDatatables(final HttpServletRequest request, final HttpServletResponse response,
			@RequestParam("appId") final String appId,
			@RequestParam(value = "appVersion", required = false, defaultValue = "0") Long appVersion,
			@RequestParam("dataListId") final String dataListId,
			@RequestParam(value = "page", required = false, defaultValue = "0") final Integer page,
		...
		try {
			...
			try {
				...
				jsonResponse.put("recordsTotal", dataList.getSize());
				jsonResponse.put("recordsFiltered", dataList.getSize());

				if (!Objects.equals(digest, currentDigest))
					jsonResponse.put("data", jsonData);

				jsonResponse.put("draw", 1);
				LogUtil.info(getClass().getName(), "[SERVER SIDE DATATABLES] "+jsonResponse.toString());
				response.setStatus(HttpServletResponse.SC_OK);
				response.getWriter().write(jsonResponse.toString());
			} catch (JSONException e) {
				throw new ApiException(HttpServletResponse.SC_INTERNAL_SERVER_ERROR, e);
