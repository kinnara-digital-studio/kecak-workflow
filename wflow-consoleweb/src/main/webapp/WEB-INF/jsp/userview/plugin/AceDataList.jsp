...
<div class="datalist-body-content row">
    <div class="col-xs-12">
        <c:if test="${!empty properties.customHeader}">
            <h3 class="header smaller lighter blue">${properties.customHeader}</h3>
        </c:if>
        <div class="clearfix">
        ...
        <c:if test="${!empty properties.error}">
            <div class="table-header">
                Error generating the data list. ${properties.error}
            </div>
        </c:if>
        ...
        <c:if test="${!empty dataList}">
            ...
            <div class="dataList">
                ...
                <c:catch var="dataListException">
                                ...
                                <form name="filters_${dataListId}" id="filters_${dataListId}" action="?" method="POST">
                                    <table border="0" width="100%">
                                     ...
                                     <span class="filter-cell">
                                         <input type="submit" class="btn btn-sm btn-primary" value="<fmt:message key="general.method.label.show"/>"/>
                                     </span>
                                </form>
                            </c:if>
                        </c:when>
                    ...
                    <form name="form_${dataListId}" action="?${queryString}" method="POST">
                            ...
                            <div class="actions bottom ${buttonFloat}">
                                <c:forEach items="${dataList.actions}" var="action">
                                    <c:if test="${!(empty dataListRows[0] || checkboxPosition eq 'no') || action.visibleOnNoRecord}">
                                        ...
                                        <button name="${dataList.actionParamName}" class="btn btn-sm btn-primary" value="${action.properties.id}" ${buttonConfirmation}><c:out value="${action.linkLabel}" escapeXml="true"/></button>
                                    </c:if>
                                </c:forEach>
                            </div>
                        </c:if>
                        ...
                        <display:table id="${dataListId}" name="dataListRows" pagesize="${dataListPageSize}" class="table table-bordered table-hover" export="true" decorator="decorator" excludedParams="${dataList.binder.primaryKeyColumnName}" requestURI="?" sort="external" partialList="true" size="dataListSize">
                            <c:if test="${checkboxPosition eq 'left' || checkboxPosition eq 'both'}">
                                <c:choose>
                                    ...
                                    <c:otherwise>
                                        <display:column headerClass="select_checkbox" class="select_checkbox" property="checkbox" media="html" title="<label class='pos-rel'><input type='checkbox' onclick='toggleAll(this)' class='ace'/><span class='lbl'></span></label>" />
                                    </c:otherwise>
                                </c:choose>
                            </c:if>
                        ...
                        <div class="well well-sm" id="export-list"></div>
                        <!-- Display Buttons -->
            ...
            <script>
                ...
                $(document).ready(function() {
                    ...
                    $("span.pagebanner").appendTo("#export-list");
                    $("span.pagelinks").appendTo("#export-list");
                    $("div.exportlinks").appendTo("#export-list");
                });
                function toggleAll(element) {
                    var table = $(element).parent().parent().parent().parent().parent();
